...
(function(){
	...
	CompassHelper.prototype.watchHeading = function(onSuccess, onError) {
		...
		if (this.availability.browser && this.browserCompass.falsePostivieDetected) {
			LOG.warn("browserCompass not available!");
			// re-init
			this.availability.browser = false;
			this.available = false;
			this.init();
		}
		...
		if (compassType == 'native') {
			var watchID = this.nativeCompass.watchHeading(function(cordovaHeading) {
